...
public class Queen extends Piece {
	...
	public boolean isValidMove(Tile[][] board, int destX, int destY) {
		...
		/* First check the straight paths */
		...
		/* Then check the diagonal paths */
		//holds the differences in distances to calculate diagonals
		xDistance = Math.abs(destX - this.x);
		yDistance = Math.abs(destY - this.y);
				
		if (this.x < destX && this.y < destY && xDistance == yDistance) { //down right
			/* Loop through the path and check if there are any pieces in the way */
			for (int i = 1; i < xDistance; i++) {
				if (!board[this.x + i][this.y + i].isEmpty())
					return false;
			}
			return true;
		} else if (this.x > destX && this.y < destY && xDistance == yDistance) { //down left
			for (int i = 1; i < xDistance; i++) {
				if (!board[this.x - i][this.y + i].isEmpty())
					return false;
			}
			return true;
		} else if (this.x < destX && this.y > destY && xDistance == yDistance) { //up right
			for (int i = 1; i < xDistance; i++) {
				if (!board[this.x + i][this.y - i].isEmpty())
					return false;
			}
			return true;
		} else if (this.x > destX && this.y > destY && xDistance == yDistance) { //up left
			for (int i = 1; i < xDistance; i++) {
				if (!board[this.x - i][this.y - i].isEmpty())
					return false;
			}
		...
		//otherwise the move is not in a diagonal or a straight path
		return false;
	}
}
